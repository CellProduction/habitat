...
@ConditionalOnProperty(prefix = "init.db", name = "enabled", havingValue = "true")
public class InitDatabaseService_V1 {
    ...
    @Autowired
    public InitDatabaseService_V1(GridFsTemplate gridFsTemplate, CellService cellService,
                                  CellFeaturesService featuresService, RegionRepository regionRepository) {
    ...
    private void storeCellBackgroundImage(Integer x, Integer y) {
        ...
        try (InputStream imageInputStream = Files
                .newInputStream(new File(fullFilename(x, y)).toPath(), StandardOpenOption.READ)) {
